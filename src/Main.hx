...
class Main {
	...
	function readme3() {
		...
		var html = html([
			template({name: 'footwo'}, [
				div({clas: 'container'}, [p({text: 'use: clas'}),]),
				div({_class: 'container'}, [p({text: 'use: _class'}),]),
				div({'class': 'container'}, [p({text: 'use: "class"'}),]),
			])
		]);
		Out.write(html, templateName);
	}
	...
	function init0() {
		...
		var html = html([template({name: templateName})]);
		Out.write(html, templateName);
	}
	...
	function init1() {
		...
		var html = html([
			template({name: templateName}, [div({clas: 'container'}, [h1({text: templateName})])])
		]);
		Out.write(html, templateName);
	}
	...
	function vue1() {
		...
		var html = vue([div({id: 'app'}, [p({text: '{{ message }}'})])]);
		Out.write(html, templateName);
	}
	...
	function vue2() {
		...
		var html = vue([
			div({id: 'app-2'}, [
				...
				ol({}, [li({text: 'list item 1'}), li({text: 'list item 2'}), li({text: 'list item 3'}),]),
				ol({}, [
					el('li', {
						"v-for": "todo in todos",
						text: "{{ todo.text }}",
				...
				el('app-nav', {}, [
					el('app-view', {}, [el('app-sidebar', {text: ''}), el('app-content', {text: ''}),]),
				]),
	...
	function vue3() {
		...
		var html = vue([div({id: 'app'}, [span({'v-if': 'seen'})])]);
		Out.write(html, templateName);
	}
	...
	function swig1() {
		...
		var html = swig([
			...
			ul({}, [
				...
				// comment('the next needs more love'),
				// li({text:'{{ author }}', print:'{% if loop.first %}', _class:'first', print_end:'{% endif %}'}),
				swig_if('loop.first', [swig_comment('if loop'),]),
			])
		]);
		Out.write(html, templateName);
	}
